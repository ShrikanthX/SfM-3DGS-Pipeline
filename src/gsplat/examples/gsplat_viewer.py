...
class GsplatViewer(Viewer):
    ...
    This subclasses nerfview.Viewer and adds a custom "Gsplat" folder
    inside the standard rendering UI.
    """
    ...
    def _init_rendering_tab(self):
        ...
        # nerfview expects this folder to exist when its own _populate_rendering_tab runs
        self._rendering_folder = self.server.gui.add_folder("Rendering")

    def _populate_rendering_tab(self):
        server = self.server
        # Put all gsplat-specific controls under a sub-folder of the rendering folder
        with self._rendering_folder:
            with server.gui.add_folder("Gsplat"):
                ...
                eps2d_slider = server.gui.add_number(
                    ...
                    step=0.01,
                    hint=(
                        "Epsilon added to the eigenvalues of projected 2D "
                        "covariance matrices."
                    ),
                )
                ...
                inverse_checkbox = server.gui.add_checkbox(
                    ...
                    disabled=True,
                    hint="Inverse the depth (or alpha).",
                )
        ...
        # Let nerfview add its standard rendering controls under this folder/tab
