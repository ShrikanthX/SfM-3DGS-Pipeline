...
def run_triangulation(
    ...
    """
    Triangulate using the chosen backend.
    For this helper we only implemented the FastMap branch, because that's what your pipeline uses.
    """
    model_path.mkdir(parents=True, exist_ok=True)

    if sfm_backend == "fastmap":
        from .fastmap_integration import run_fastmap as _run_fastmap

        cm = (camera_model or "").upper()
        pinhole = cm in {"PINHOLE", "SIMPLE_PINHOLE"}
    ...
    # If someone calls this with "colmap", we could add a colmap triangulation here.
    # For now, just log it.
    logger.info("run_triangulation called with sfm_backend=%s, but only fastmap is implemented here.", sfm_backend)
    return None
...
def main(
    ...
    if reconstruction is not None:
        ...
        logger.info("Finished the triangulation.")
